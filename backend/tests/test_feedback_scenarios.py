...
@pytest.mark.asyncio
async def test_weak_answer_fallback_scenario():
    days = sorted({turn["day"] for turn in WEAK_HISTORY})
    fb = feedback_engine._fallback_feedback("Weak Candidate", days, WEAK_HISTORY)

    assert fb.verdict in {"NO_HIRE", "BORDERLINE"}
    assert fb.strengths == []
    assert len(fb.topic_scores) == len(days)
...
@pytest.mark.asyncio
async def test_strong_answer_fallback_scenario():
    days = sorted({turn["day"] for turn in STRONG_HISTORY})
    fb = feedback_engine._fallback_feedback("Strong Candidate", days, STRONG_HISTORY)

    assert fb.verdict in {"HIRE", "STRONG_HIRE"}
    assert len(fb.strengths) >= 2
    assert len(fb.topic_scores) == len(days)
    assert all(score.score >= 55 for score in fb.topic_scores)
...
async def test_honest_fallback_feedback_degradation():
    ...
    assert "brief" in fb.summary.lower() or "limited depth" in fb.summary.lower()
    assert any(
        "incomplete" in g.lower() or "superficial" in g.lower() or "lacked" in g.lower()
        for g in fb.gaps
    )
